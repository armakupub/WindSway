...
// leaf mask and shade off), 0 low (crown bend only). Once per session, with
// the debug log on, the tree pass is timed against the quad area it shaded
// and logged; ns per pixel is a property of the card, not of the scene.
// Nothing is switched on that number.
final class TreeDetail {
    ...
    static void set(int level) {
        ...
        active = tier;
        TreeRenderer.setQuality(tier >= MEDIUM, tier == HIGH, tier >= MEDIUM, tier == HIGH, tier == HIGH);
    }
